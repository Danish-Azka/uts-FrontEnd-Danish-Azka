...
export const getCartByShopId = async () => {
  try {
    const d = localStorage.getItem("idBuyer"); 
    if (!d) {
      throw new Error("d tidak ditemukan di localStorage");
    }

    const response = await axios.get(`http://localhost:3009/product/shop/${d}`);
    return response.data;
  } catch (error) {
