...
export const getCartByShopId = async () => {
  try {
    const Buyerd = localStorage.getItem("idBuyer"); 
    if (!Buyerd) {
      throw new Error("Buyerd tidak ditemukan di localStorage");
    }

    const response = await axios.get(`http://localhost:3009/product/shop/${Buyerd}`);
    return response.data;
  } catch (error) {
