...
const ModalFormTransaksi = ({ onClose }) => {
  ...
  }, [mobil.tanggalPeminjaman, mobil.batasPeminjaman]);

  useEffect(() => {
    const fetchMobilPrice = async () => {
      if (mobil.MobilId) {
        try {
          const response = await getMobilById(mobil.MobilId); 
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createTransaksi(mobil);  
      console.log('Formulir dikirim:', mobil);
      onClose(); 
    } catch (err) {
