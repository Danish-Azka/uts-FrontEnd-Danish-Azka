...
const Login = ({ apiUrl }) => {
  ...
  // Function untuk fetch user dari API
  const getUsers = async () => {
    try {
      const response = await axios.get(apiUrl); // Menggunakan apiUrl dari props
  ...
  // Timer untuk menghilangkan alert setelah 2 detik
  useEffect(() => {
    if (alertShow) {
      ...
      <div className="flex items-center justify-center p-4">
        <div className="bg-white p-6 rounded-lg shadow-lg w-[400px]">
          ...
          <h6 className="text-center text-sm mt-2">
            <span className="px-2 py-1 bg-gray-200 rounded">To GearUP</span>
          </h6>
          <div className="mt-6">
