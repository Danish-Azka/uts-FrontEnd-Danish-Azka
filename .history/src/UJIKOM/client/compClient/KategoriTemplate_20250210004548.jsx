...
export const KategoriTemplate = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      const filteredProducts = data.filter(product => product.category === "Sparepart Kendaraan");
    ...
    <div className="w-full h-full bg-[#f5f5f5]">
      <div>
        ...
        <div className="w-full p-5 h-[350px] gap-3 flex justify-center items-center">
          <div className="w-8/12 h-full bg-yellow-500 rounded-t-lg">
            <div className="h-1/6 w-full bg-[#1D1E20]flex justify-between items-center px-3">
              <p className="font-bold text-xl text-whoit">G E A R U P</p>
              <p className="font-semibold text-white">Kategori : Sparepart Kendaraan</p>
            </div>
          </div>
        </div>

        <div className="w-full h-fit gap-3 flex justify-center items-center">
          <div className="w-10/12 h-full bg-white shadow-lg rounded-lg p-3">
            <div className="grid grid-cols-3 gap-4 w-full h-full">
              {loading ? (
                ...
                <p className="text-white text-center col-span-3">Tidak ada produk dalam kategori ini.</p>
              )}
            </div>
          </div>
        </div>
