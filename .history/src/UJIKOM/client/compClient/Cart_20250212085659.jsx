...
const Cart = ({ isCartOpen, onClose }) => {
  ...
  const totalBiaya = selectedItems.reduce((acc, id) => {
    const item = data.find(cart => cart.id === id);
    return acc + (item ? item.Totalharga : 0);
  }, 0) + (selectedItems.length > 0 ? shippingCost : 0);
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-slate-300 rounded-lg h-[700px] w-[900px] shadow-lg">
        ...
        <div className='w-full max-h-full p-3 flex gap-3 justify-between items-center h-[640px]'>
            ...
            {data.length > 0 ? (
              data.map((item) => (
                <div key={item.id} className='h-[250px] bg-white w-full flex flex-col p-3 rounded-lg shadow-md'>
                  ...
                  <p className='text-xl font-semibold'>{item.Product?.nama} x {item.jumlah}</p>
          ...
          <div className='bg-white max-h-full h-[300px] items-start p-3 rounded-xl w-5/12'>
            ...
            <div className='mt-3'>
              {selectedItems.length > 0 ? (
                selectedItems.map((id) => {
                  const item = data.find(cart => cart.id === id);
                  return (
                    <p key={id} className='text-l'>{item?.Product?.nama} x {item?.jumlah} - IDR {item?.Totalharga.toLocaleString()}</p>
                  );
                })
              ) : (
                <p className='text-gray-600'>Tidak ada item yang dipilih</p>
              )}
              <p className='text-l'>Shipping Costs: IDR {selectedItems.length > 0 ? shippingCost.toLocaleString() : 0}</p>
            </div>
            <div className='w-full h-20 border border-black mt-5 p-3 rounded-xl flex items-center'>
              <p className='text-xl'>Total: IDR {totalBiaya.toLocaleString()}</p>
            </div>
            <div className='flex justify-end items-center mt-2'>
              <button className='px-3 py-1 bg-black text-white rounded-xl' disabled={selectedItems.length === 0}>CheckOut</button>
            </div>
          </div>
        </div>
      </div>
