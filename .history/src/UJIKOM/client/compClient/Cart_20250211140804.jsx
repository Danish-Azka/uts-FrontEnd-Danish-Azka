...
const CartModal = ({ product, isOpen, onClose }) => {
  const [jumlka, setjumlka] = useState(1);
  const [loading, setLoading] = useState(false);

  if (!isOpen) return null;

  const totalHarga = jumlka * product.harga;
  ...
  const handleConfirm = async () => {
    ...
    try {
      const cartData = {
        ...
        BuyerId: 1, // Ubah dengan ID pembeli yang benar (misalnya dari state user)
        jumlah: jumlka,
      ...
      <div className="bg-white p-5 rounded-lg w-[400px] shadow-lg">
        ...
        <div className="mb-3">
          ...
          <input
            ...
            min="1"
            value={jumlka}
            onChange={(e) => setjumlka(Number(e.target.value))}
