...
import { getCartByBuyerId, deleteCart, cartUpdate } from '../../../service/apiCart';
...
const Cart = ({ isOpen, onClose }) => {
  ...
  const handleUpdateQuantity = async (cartId, newQuantity, hargaPerItem) => {
    ...
    try {
      ...
      await cartUpdate(cartId, { jumlah: newQuantity, Totalharga: updatedTotalHarga });
      setData(data.map(item => item.id === cartId ? { ...item, jumlah: newQuantity, Totalharga: updatedTotalHarga } : item));
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
      <div className="bg-slate-300 rounded-lg h-[600px] w-[700px] shadow-lg">
        <div className="flex justify-between h-1/6 p-5 bg-black rounded-t-lg items-center border-b pb-2 mb-3">
            ...
            data.map((item, index) => (
              <div key={index} className='h-[250px] bg-white w-full flex flex-col'>
                ...
                <div className='flex flex-row gap-2 p-3'>
                  <div className='w-1/3'>
                    <img className="w-full h-4/6 bg-yellow-500" src={item.Product?.gambar} alt="" />
                  </div>
                  <div className='w-2/3 h-5/6 flex flex-col justify-start items-start gap-2'>
                    ...
                    <p>Harga Per Item: {item.Product?.harga.toLocaleString()}</p>
                    <p>Total: IDR {item.Totalharga.toLocaleString()}</p>
                    <div className='flex items-center gap-2'>
                      <button className='bg-red-500 text-white px-2 rounded' onClick={() => handleUpdateQuantity(item.id, item.jumlah - 1, item.Product?.harga)}>-</button>
                      <p>{item.jumlah}</p>
                      <button className='bg-green-500 text-white px-2 rounded' onClick={() => handleUpdateQuantity(item.id, item.jumlah + 1, item.Product?.harga)}>+</button>
                    </div>
                  </div>
                </div>
              </div>        
