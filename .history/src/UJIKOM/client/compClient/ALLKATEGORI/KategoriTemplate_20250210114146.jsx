  ...
  return(
    <div className='w-full h-full cols grid-cols-4 gap-4 p-5'>
      <div className='rounded-xl w-1/4 shadow-lg h-[150px] flex flex-col justify-center items-center'>
        <img className='w-[100px]' src={Toko.gambar} alt="" />
          ...
          <div className="w-8/12 h-full flex flex-col bg-white rounded-lg">
            ...
            <div className='w-full h-5/6 flex justify-center items-center'>
              <div className='w-11/12 h-full'>
                {toko && toko.map((item, index) => {
                  <ShopSell key={Toko.id} Toko={}
                })}
              </div>
            </div>
          </div>
          ...
          <div className="w-10/12 h-full bg-white shadow-lg rounded-t-lg p-3">
            <div className="grid grid-cols-3 gap-4 w-full h-full">
              {loading ? (
                ...
                products.map((product) => <ProductCard key={product.id} proTokoduct={product} />)
              ) : (
