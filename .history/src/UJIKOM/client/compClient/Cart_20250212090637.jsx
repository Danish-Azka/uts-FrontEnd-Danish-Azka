...
const Cart = ({ isCartOpen, onClose }) => {
  ...
  const totalHarga = selectedCartItems.reduce((total, item) => total + item.Totalharga, 0) + (selectedCartItems.length > 0 ? 100000 : 0);
          ...
          <div className='bg-white max-h-full h-[300px] items-start p-3 rounded-xl w-5/12'>
            ...
            {selectedCartItems.map((item) => (
              <div key={item.id} className='mt-3 flex items-center gap-3'>
                ...
                <p>{item.Product?.nama} x {item.jumlah}</p>
              </div>
            ))}
            ...
            <div className='flex justify-end items-center mt-2'>
              <div className='px-3 py-1 bg-black text-white rounded-xl cursor-pointer'>CheckOut</div>
            </div>
          </div>
        </div>
      </div>
