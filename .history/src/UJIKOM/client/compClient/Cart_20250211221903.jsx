            ...
            data.map((item, index) => (
              <div key={index} className='h-[250px] bg-white w-full flex flex-col'>
                <div className='flex justify-between w-full h-15 gap-5 items-center p-3'>
                  <div> <img className='w-10 h-10 rounded-full border border-black' src={item.Shop?.gambar} alt="" />
                  <p>{item.Shop?.nama}</p> </div>
                  <div className='items-end'><Trash2Icon/></div>
                </div>

                <div className='flex flex-row  gap-2 p-3'>
                  <div className='w-1/3 ]'><img  className="w-full h-5/6 bg-yellow-500"src={item.Product?.gambar} alt="" /></div>
