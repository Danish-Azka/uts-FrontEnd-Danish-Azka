...
const CartModal = ({ product, isOpen, onClose }) => {
  const [quantity, setJumlahy] = useState(1);
  const [loading, setLoading] = useState(false);
  const id = localStorage.getItem("idBuyer")

  if (!isOpen) return null;

  const totalHarga = quantity * product.harga;
  ...
  const handleConfirm = async () => {
    ...
    try {
      const cartData = {
        ...
        BuyerId: {id}, // Ubah dengan ID pembeli yang benar (misalnya dari state user)
        jumlah: quantity,
      ...
      <div className="bg-white p-5 rounded-lg w-[400px] shadow-lg">
        ...
        <div className="mb-3">
          ...
          <input
            ...
            min="1"
            value={quantity}
            onChange={(e) => setJumlahy(Number(e.target.value))}
