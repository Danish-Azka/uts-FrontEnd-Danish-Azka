...
const fetchTransaksi = () => {
  ...
  useEffect(() => {
    if (formData.tanggalPengembalian && formData.batasPeminjaman) {
      const returnDate = new Date(formData.tanggalPengembalian);
      const batas = new Date(formData.batasPeminjaman);

      if (returnDate > batas) {
        const diffInMs = returnDate - batas;
        const diffInDays = Math.ceil(diffInMs / (1000 * 60 * 60 * 24));
        setDenda(diffInDays * 50000);
      } else {
        setDenda(0);
      }
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await createPengembalian({...formData, denda});  
